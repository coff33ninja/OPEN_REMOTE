...
Keyboard is text injection only. There is no per-key send, no key down/up, no modifiers, no shortcuts like Alt+Tab or Ctrl+Shift+Esc, no arrows, no function keys, no backspace/enter/tab controls, and no clipboard sync. That is explicit in keyboard_screen.dart and plugin.go.
