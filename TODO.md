...
- Services are now implemented for Windows with list, start/stop/restart actions, startup type, and status reason, but there is no daemon/service health detail view, history, or non-Windows support yet. See android/lib/features/services/services_screen.dart and agent/internal/system/services_windows.go.
