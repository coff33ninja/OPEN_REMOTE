...
- Current active items are mostly absent. There is no active window list, no foreground app, no media-session metadata, no currently playing track, no window titles, no network throughput telemetry, and no live process updates. The agent is command-oriented, not state-oriented, in agent/internal/server/websocket.go. System CPU/GPU/memory/disk snapshots now exist but are polling-based.
